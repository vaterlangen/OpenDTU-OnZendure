...
uint16_t Provider::setOutputLimit(uint16_t limit) const
{
    ...
    if (!_stats->_output_limit.has_value() || *_stats->_output_limit != limit) {
        ...
        DTU_LOGD("Adjusting outputlimit from %" PRIu32 " W to %" PRIu16 " W", _stats->_output_limit.value_or(0), limit);
    }
...
uint16_t Provider::setInverterMax(uint16_t limit) const
{
    ...
    if (_stats->_inverse_max != limit) {
        ...
        DTU_LOGD("Adjusting inverter max output from %" PRIu32 " W to %" PRIu16 " W", _stats->_inverse_max.value_or(0), limit);
    }
