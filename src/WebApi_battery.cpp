...
void WebApiBatteryClass::init(AsyncWebServer& server, Scheduler& scheduler)
{
    ...
    _server->on("/api/battery/list", HTTP_GET, static_cast<ArRequestHandlerFunction>(std::bind(&WebApiBatteryClass::onBatteryList, this, _1)));
    _server->on("/api/battery/add", HTTP_POST, static_cast<ArRequestHandlerFunction>(std::bind(&WebApiBatteryClass::onBatteryAdd, this, _1)));
    _server->on("/api/battery/edit", HTTP_POST, static_cast<ArRequestHandlerFunction>(std::bind(&WebApiBatteryClass::onBatteryEdit, this, _1)));
    _server->on("/api/battery/del", HTTP_POST, static_cast<ArRequestHandlerFunction>(std::bind(&WebApiBatteryClass::onBatteryDelete, this, _1)));
    _server->on("/api/battery/order", HTTP_POST, static_cast<ArRequestHandlerFunction>(std::bind(&WebApiBatteryClass::onBatteryOrder, this, _1)));
}
...
void WebApiBatteryClass::onBatteryAdd(AsyncWebServerRequest* request)
{
    ...
    if (root["name"].as<String>().length() == 0 || root["name"].as<String>().length() > BAT_MAX_NAME_STRLEN) {
        retMsg["message"] = "Name must between 1 and " + std::to_string(BAT_MAX_NAME_STRLEN) + " characters long!";
    ...
    if (!battery) {
        retMsg["message"] = "Only " + std::to_string(BAT_MAX_COUNT) + " batteries are supported!";
        retMsg["code"] = WebApiError::GenericNoValueFound;
